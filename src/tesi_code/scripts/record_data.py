...
class OdometryRecorder(Node):
    def __init__(self):
        ...
        self.declare_parameter("file_name", "odom_result")
        ...
        self.filepath=os.path.join(get_package_share_directory('tesi_code'), "result")
    ...
    def listener_callback_tf(self, msg):
        ...
        self.tf_data.append(data_odom_base_link.extend(data_map_odom))
    ...
    def save_data(self):
        ...
        while (i<(limit+2)):
            ...
            my_file = Path(f"{self.filepath}{self.file_name}_odom_{i:03.0f}.csv")
            if my_file.is_file():
                i+=1
            else:
                self.file_name=f"{self.file_name}_odom_{i:03.0f}.csv"
                with open(self.filepath+self.file_name, mode="w", newline="") as file:
        ...
        while (i<(limit+2)):
            ...
            if my_file.is_file():
                ...
                self.file_name=f"{self.file_name}_tf_{i:03.0f}.csv"
                with open(self.filepath+self.file_name, mode="w", newline="") as file:
                    writer = csv.writer(file)
                    writer.writerow(["Time (s)", "X (m)", "Y (m)", "heading"])
                    writer.writerows(self.odometry_data)
